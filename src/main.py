...
from src.local_themes import cluster_local_stories, format_local_spotlight
...
def save_summaries(
    ...
    data = {
        ...
        "local_themes": local_themes or [],
        "local_articles": local_articles or [],
        "local_theme_count": len(local_themes) if local_themes else 0
    }
...
"""
    ...
    local_section = ""
    if local_themes:
        local_section = format_local_spotlight(local_themes)

    footer = """---

*Reply with the numbers you want (e.g., "1, 3, 5, 7, 9, 11") and I'll send you the final formatted issue.*
*Local Spotlight themes will be automatically included.*

*Generated by PulseK12 Newsletter Engine*
"""

    return header + body + local_section + footer
...
def run_pipeline(
    ...
    # Generate local themes
    print("\n  Clustering local stories by theme...")
    local_themes = []
    if local_articles:
        local_themes = cluster_local_stories(local_articles, max_themes=2)
        if local_themes:
            print(f"  Generated {len(local_themes)} local themes:")
            for theme in local_themes:
                print(f"    - {theme.get('theme_title', 'Unknown')}")
        else:
            print("  No common themes found in local stories")
    stats["local_themes"] = len(local_themes)

    # Save summaries for finalize.py
    print("\nSaving summaries for finalization...")
    save_summaries(national_summaries, local_themes, local_articles)

    # Format the menu
    print("\nFormatting menu...")
    menu_content = format_menu(national_summaries, local_themes)
    ...
    print(f"  Local themes generated: {stats.get('local_themes', 0)}")
